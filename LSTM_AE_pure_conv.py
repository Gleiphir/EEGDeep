...
class LSTMEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv1d(5  , DIM, 5, ) # 5 * 36 -> 64 * 32
        self.conv2 = nn.Conv1d(DIM, DIM1 , 3, stride=2, padding=1) # 64 * 32 -> 32 * 16
        self.conv3 = nn.Conv1d(DIM1 , DIM2, 3, stride=2, padding=1) # 32 * 16 -> 16 * 8
        self.fc1 = nn.Linear(DIM * (32 // 4**2), bottleneck_channel)
...
class LSTMDecoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.ConvTranspose1d(DIM, DIM1, kernel_size=3, stride=2, padding=0)
        self.conv2 = nn.ConvTranspose1d(DIM1, DIM2, kernel_size=4, stride=2, padding=1)
        self.conv3 = nn.ConvTranspose1d(DIM2, 5  , kernel_size=4, stride=2, padding=1)
        #self.ln1 = nn.GroupNorm(1, DIM)
        #self.ln2 = nn.GroupNorm(1, DIM)
        #self.ln3 = nn.GroupNorm(1, DIM)
    ...
    def forward(self, x):  # BxDIM
        x = self.fc1(x)
        x = x.view(-1, DIM, 4)     # Bx 64 x 4
        x = F.relu(self.conv1(x))  # Bx 32 x 9
        x = F.relu(self.conv2(x))  # Bx 16 x 18
        x = self.conv3(x)  # B x 5 x 36
        return x.transpose(1,2) #B x 36 x 5
